import { IntentWeights } from "./types";
...
function parseBudget(text: string): { minBudget?: number; maxBudget?: number } {
  ...
  // “under 10 lakhs”, “below 8L”, “< 12 lakh”
  const lakhMatch = lower.match(/(\d+(\.\d+)?)\s*(lakh|lakhs|l)/);
  if (lakhMatch) {
    const value = parseFloat(lakhMatch[1]);
    const amount = value * 100000;
    if (lower.includes("under") || lower.includes("below") || lower.includes("<=") || lower.includes("<")) {
      maxBudget = amount;
    } else if (lower.includes("above") || lower.includes("over") || lower.includes(">=") || lower.includes(">")) {
      minBudget = amount;
    } else {
      maxBudget = amount;
    }
...
export function parseIntentDeterministic(query: string): IntentWeights {
  ...
  let safety = 1;
  if (lower.includes("safety") || lower.includes("safe") || lower.includes("family")) {
    safety = 3;
  }

  let mileage = 1;
  if (lower.includes("mileage") || lower.includes("fuel") || lower.includes("economy")) {
    mileage = 3;
  }
...
// Optional hook for future LLM-based parsing; currently just returns null.
async function parseIntentWithLLM(_query: string): Promise<IntentWeights | null> {
  // In future you can plug in an LLM here using process.env.LLM_API_KEY
  return null;
}
